...
def getallsubdomainIPS(domain):
    alladdresses = []
    for x in getSubdomain(domain)[1]:
...
def setOrganizationUsers(org_id):
    """Set user access level at newly created stakeholder at CSG portal via API."""
    for user in getalluserinfo():
        userrole = user["role_id"]
        user_id = user["user_id"]
        print(f"The userrole {userrole} and the user_id {user_id}")
        if (
...
def index():
    ...
    if formExternal.validate_on_submit():
        ...
        allValidIP = validateIP(custIP)

        # print(f'{cust}')
        # print(f'{custIP}')
        # print(f'{custDomainAliases}')
        # print(f'{custRootDomain}')
        # print(f'{custSubDomain}')
            ...
            if cust not in allDomain.values():
                ...
                if setStakeholder(cust):
                    ...
                    if setCustRootDomain(cust, custRootDomainValue, allDomain):
                        ...
                        if setCustSubDomain(allSubDomain, rootUUID, cust):
        ...
        # else:
        #     if cust not in allDomain:
        #         # print('Got to where it should flash')
        #         flash(f"You successfully submitted a new customer {cust} ",
        #               "success")
        #         # setCustomer(cust, custIP)
        #     else:
        #         flash(f'The customer already exists. {cust}', 'warning')
...
# 107.85.192.20/32,107.85.192.126/32
